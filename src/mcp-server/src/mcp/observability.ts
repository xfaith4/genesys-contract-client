...
export type ToolResultStatus = "ok" | "error" | "denied" | "timeout" | "cancelled";
...
function makeHistogram(bucketsMs: number[]): Histogram {
  return {
    bucketsMs,
...
function observeHistogram(histogram: Histogram, valueMs: number): void {
  histogram.count += 1;
...
function truncatedSessionId(sessionId: string | undefined): string | undefined {
  if (!sessionId) return undefined;
  if (sessionId.length <= 12) return sessionId;
  return `${sessionId.slice(0, 8)}...${sessionId.slice(-4)}`;
}

export function normalizeErrorClass(mapped: { status: number; message: string; details?: unknown }): NormalizedErrorClass {
  const message = String(mapped.message || "").toLowerCase();

  if (message.includes("cancelled") || message.includes("aborted")) return "CLIENT_CANCELLED";
  if (mapped.status === 429) return "RATE_LIMITED";
  if (mapped.status === 408 || mapped.status === 504 || message.includes("timed out")) return "TIMEOUT";
  if (mapped.status === 401) return "AUTH_FAILED";
  if (mapped.status === 403) {
    if (message.includes("policy") || message.includes("not allowed") || message.includes("disabled")) return "POLICY_DENIED";
    return "AUTH_FAILED";
  }
  if (mapped.status === 404) return "NOT_FOUND";
  if (mapped.status === 400 && message.includes("schema validation")) return "SCHEMA_VALIDATION_FAILED";
  if (mapped.status === 400) return "BAD_REQUEST";
  if (mapped.status >= 500) return "UPSTREAM_5XX";
  if (mapped.status >= 400) return "UPSTREAM_4XX";
  return "INTERNAL_ERROR";
}

export function mapErrorClassToResultStatus(errorClass: NormalizedErrorClass): ToolResultStatus {
  if (errorClass === "POLICY_DENIED") return "denied";
  if (errorClass === "TIMEOUT") return "timeout";
  if (errorClass === "CLIENT_CANCELLED") return "cancelled";
  return "error";
}
...
export class ObservabilityStore {
  ...
  private readonly httpRequestDuration = new Map<string, Histogram>();

  private readonly mcpRequestsTotal = new Map<string, number>();

  private readonly mcpErrorsTotal = new Map<string, number>();

  private readonly mcpDeniedTotal = new Map<string, number>();

  private readonly toolCallTotals = new Map<string, number>();

  private readonly mcpRequestDuration = new Map<string, Histogram>();

  private readonly sessionDuration = makeHistogram(SESSION_DURATION_BUCKETS_MS);

  private readonly recentToolCalls: JsonObject[] = [];
  ...
  recordSessionClosed(durationMs: number): void {
  ...
  buildStatusSnapshot(input: StatusSnapshotInput): JsonObject {
  ...
  renderPrometheus(activeSessions: number): string {
    const lines: string[] = [];
    ...
    for (const [key, count] of this.httpRequestsTotal.entries()) {
      ...
      lines.push(
        `mcp_http_requests_total{method="${label(method)}",path="${label(path)}",status="${label(status)}"} ${count}`,
      );
    }
    ...
    for (const [key, histogram] of this.httpRequestDuration.entries()) {
      ...
      for (let idx = 0; idx < histogram.bucketsMs.length; idx += 1) {
        ...
        lines.push(
          `mcp_http_request_duration_seconds_bucket{method="${label(method)}",path="${label(path)}",le="${toSeconds(histogram.bucketsMs[idx]).toFixed(3)}"} ${cumulative}`,
        );
      }
    ...
    for (const [key, count] of this.mcpRequestsTotal.entries()) {
      ...
      lines.push(
        `mcp_requests_total{method="${label(method)}",tool="${label(tool)}",status="${label(status)}"} ${count}`,
      );
    }
    ...
    for (const [key, histogram] of this.mcpRequestDuration.entries()) {
      ...
      for (let idx = 0; idx < histogram.bucketsMs.length; idx += 1) {
        cumulative += histogram.bucketCounts[idx];
        lines.push(
          `mcp_request_duration_seconds_bucket{method="${label(method)}",tool="${label(tool)}",le="${toSeconds(histogram.bucketsMs[idx]).toFixed(3)}"} ${cumulative}`,
        );
      }
    ...
    for (let idx = 0; idx < this.sessionDuration.bucketsMs.length; idx += 1) {
      ...
      lines.push(
        `mcp_session_duration_seconds_bucket{le="${toSeconds(this.sessionDuration.bucketsMs[idx]).toFixed(3)}"} ${cumulative}`,
      );
    }
