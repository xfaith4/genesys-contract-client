...
function normalizeYamlList(value) {
...
function readYamlOrDefault(filePath, defaultValue) {
...
export function loadPolicyList(repoRoot, fileName) {
...
export function loadSimpleList(repoRoot, fileName) {
    const yamlPath = path.join(repoRoot, "registry", fileName);
    const parsed = readYamlOrDefault(yamlPath, []);
    const rows = normalizeYamlList(parsed);
...
export function loadLoggingPolicy(repoRoot) {
...
export function operationMatchesPolicy(op, policy) {
...
export function redactForLog(value, redactionFields) {
    ...
    if (typeof value !== "object")
        return value;
    if (Array.isArray(value))
        return value.map((x) => redactForLog(x, redactionFields));
    if (!isPlainObject(value))
...
function tokenizePath(jsonPath) {
...
function getValueAtPath(obj, jsonPath) {
...
export function summarizeRequest(operationId, params, body, loggingPolicy, redactionFields) {
