...
const operations = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "generated", "operations.json"), "utf8"));
const pagingMap = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "generated", "pagination-map.json"), "utf8"));
let tokenCache = null;
...
function assertParams(op, params) {
    ...
    for (const r of required) {
        if (!(r in params))
            throw new Error(`Missing required parameter '${r}' for operationId '${op.operationId}'.`);
    }
    for (const k of Object.keys(params)) {
        if (!declared.has(k))
            throw new Error(`Unknown parameter '${k}' for operationId '${op.operationId}'. Refusing to guess.`);
    }
}
function buildUrl(cfg, op, params) {
    ...
    for (const prm of op.parameters.filter(x => x.in === "path")) {
        if (!(prm.name in params))
            throw new Error(`Missing required path param '${prm.name}'.`);
...
app.post("/describe", (req, res) => {
    ...
    if (!op)
        return res.status(404).json({ error: "unknown operationId" });
...
app.post("/callAll", async (req, res) => {
    try {
        ...
        if (ptype === "UNKNOWN")
            throw new Error(`Unknown pagination type for ${operationId}. Add to registry or regenerate.`);
        const items = [];
        const audit = [];
        let page = 1;
        let next = null;
