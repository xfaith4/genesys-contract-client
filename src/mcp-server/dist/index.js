...
function parseAndValidateUrl(raw, fieldName, allowedHosts) {
    let url;
    try {
        url = new URL(raw);
    }
...
const DEFAULT_MAX_RUNTIME_MS = clamp(readIntEnv("DEFAULT_MAX_RUNTIME_MS", HARD_MAX_RUNTIME_MS, 1_000, HARD_MAX_RUNTIME_MS), 1_000, HARD_MAX_RUNTIME_MS);
...
const operations = readJson(path.join(REPO_ROOT, "generated", "operations.json"));
const pagingMap = readJson(path.join(REPO_ROOT, "generated", "pagination-map.json"));
const swagger = readJson(path.join(REPO_ROOT, "specs", "swagger.json"));
const definitions = (swagger?.definitions ?? {});
function loadPolicyList(fileName) {
...
function operationMatchesPolicy(op, policy) {
    if (policy.operationIds.has(op.operationId))
        return true;
    const tags = op.tags ?? [];
    for (const t of tags) {
        if (policy.tags.has(String(t).toLowerCase()))
            return true;
    }
    return false;
}
function loadSimpleList(fileName) {
    const p = path.join(REPO_ROOT, "registry", fileName);
    if (!fs.existsSync(p))
        return new Set();
    const out = new Set();
...
function redactForLog(value) {
    ...
    if (Array.isArray(value))
        return value.map((x) => redactForLog(x));
...
function validateClientConfig(cfg, source) {
...
function loadServerClientConfigFromEnv() {
...
function resolveClientConfig(input) {
    ...
    if (hasInput)
        return parseClientConfig(input);
    throw httpError(500, "Unable to resolve client configuration.");
}
function requireServerKey(req) {
    if (!SERVER_API_KEY)
        return;
...
function isOperationAllowed(op) {
    if (DENYLIST.hasEntries && operationMatchesPolicy(op, DENYLIST))
        return false;
...
async function requestWithRetry(cfg) {
    let attempt = 0;
...
function validateSchemaValue(value, schema, location) {
    ...
    if (t === "object" || (!t && (resolved.properties || resolved.required))) {
        ...
        for (const reqName of required) {
            if (!(reqName in value))
                errors.push(`${location}.${reqName}: missing required field`);
        }
    ...
    if (t === "integer" && (!Number.isInteger(value) || typeof value !== "number"))
...
function assertParams(op, params) {
...
function assertContract(op, params, body) {
    assertParams(op, params);
    assertBody(op, body);
}
function buildUrl(cfg, op, params) {
    let p = op.path;
...
// ---- Pagination safety ------------------------------------------------------
function resolveNextUrl(cfg, next) {
    return new URL(next, cfg.baseUrl).toString();
}
function assertSameHost(cfg, url) {
...
function getItemsByPath(resp, itemsPath) {
    if (!resp)
        return [];
...
function redactPagingValue(value) {
    if (value === null || value === undefined)
        return null;
...
async function callOnce(cfg, op, params, body, overrideUrl) {
...
function mapErrorToHttp(e) {
    if (e?.statusCode)
        return { status: Number(e.statusCode), message: e.message ?? String(e), details: e.details };
    const err = e;
    const status = err.response?.status;
    if (status) {
    ...
    if (err.code === "ECONNABORTED")
        return { status: 504, message: "Upstream request timed out.", details: err.code };
    return { status: 500, message: e?.message ?? String(e) };
}
function ensureObject(payload, context) {
    if (!isPlainObject(payload))
        throw httpError(400, `${context} must be an object.`);
    return payload;
}
function assertOnlyKeys(payload, allowed, context) {
...
function setPagingInBody(body, pageNumber, pageSize) {
...
function setBodyToken(body, key, value) {
    if (body === null || body === undefined)
        return { [key]: value };
    if (!isPlainObject(body))
...
function getPagingEntry(operationId, op) {
    const direct = pagingMap[operationId];
    if (direct)
        return direct;
    const catalogKey = op.catalogKey;
    if (catalogKey && pagingMap[catalogKey])
        return pagingMap[catalogKey];
...
function searchOperations(query, method, tag, limit = 25) {
    ...
    for (const op of Object.values(operations)) {
        if (!isOperationAllowed(op))
            continue;
        if (m && op.method.toUpperCase() !== m)
            continue;
        if (t && !op.tags.some((x) => String(x).toLowerCase() === t))
            continue;
...
async function executeDescribe(payload) {
...
async function executeCall(payload) {
...
async function executeCallAll(payload) {
...
app.post("/tools/invoke", async (req, res) => {
    try {
        ...
        if (tool === "genesys.describe")
            return res.json({ result: await executeDescribe(input), requestId: req.requestId });
        if (tool === "genesys.call")
            return res.json({ result: await executeCall(input), requestId: req.requestId });
        if (tool === "genesys.callAll")
            return res.json({ result: await executeCallAll(input), requestId: req.requestId });
